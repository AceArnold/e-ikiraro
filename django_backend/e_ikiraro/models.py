...
class Service(models.Model):
    ...
    service_photo = models.ImageField(
        upload_to='documents/service_photos/', null=True, blank=True)
    description = models.TextField(help_text="Description of the service")
    base_fee = models.DecimalField(
        max_digits=10, decimal_places=2, default=0.00, help_text="Base fee for the service")
...
class PassportApplication(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE,
                             related_name='passport_applications', null=True, blank=True)
...
class NationalIDApplication(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(User, on_delete=models.CASCADE,
                             related_name='national_id_applications', null=True, blank=True)
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='Pending')
    submitted_at = models.DateTimeField(default=timezone.now)
    approved_at = models.DateTimeField(null=True, blank=True)
...
class DriversLicenseApplication(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(User, on_delete=models.CASCADE,
                             related_name='drivers_license_applications', null=True, blank=True)
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default='Pending')
    submitted_at = models.DateTimeField(default=timezone.now)
    approved_at = models.DateTimeField(null=True, blank=True)
    ...
    medical_certificate = models.FileField(
        upload_to='documents/medical_certificates/')
    eye_test_certificate = models.FileField(
        upload_to='documents/eye_test_certificates/')
    national_id = models.FileField(upload_to='documents/national_ids/')
    driving_school_certificate = models.FileField(
        upload_to='documents/driving_school_certificates/', blank=True, default='')
...
class Payment(models.Model):
    ...
    passport_application = models.ForeignKey(
        PassportApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='payments')
    national_id_application = models.ForeignKey(
        NationalIDApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='payments')
    drivers_license_application = models.ForeignKey(
        DriversLicenseApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='payments')
...
class Document(models.Model):
    ...
    passport_application = models.ForeignKey(
        PassportApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='documents')
    national_id_application = models.ForeignKey(
        NationalIDApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='documents')
    drivers_license_application = models.ForeignKey(
        DriversLicenseApplication, on_delete=models.SET_NULL, null=True, blank=True, related_name='documents')
